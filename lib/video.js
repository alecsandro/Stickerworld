...
class Video extends AsyncConstructor {
    ...
    async resize(type) {
        try {
            let complexFilter = `scale=512:512:force_original_aspect_ratio=decrease,fps=10 , pad=512:512:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`;

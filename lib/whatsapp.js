...
class Message_was extends AsyncConstructor {
    constructor(client, message) {
        super(async () => {
            ...
            if (this.message.isGroupMsg) {
                ...
                //tools.conlog_info_force(`checkMessage`);
                check = await this.checkMessage();
            }

            if (check !== "Video-reject" && check !== "Url-Reject" && check !== "Video-Url-Reject" && check !== "Url-No-Detected") {
                tools.conlog_info_force(check == undefined ? 'Tipo de midia invalido': check)
                if (check != undefined) 
                {
            ...
            switch (check) {
                ...
                case "Url-No-Detected":
                    ...
                    if (config.notifyRejectionStickerGeneration) {
                        tools.conlog_info_force(`notifyRejectionStickerGeneration`);
    ...
    async sendVideo(file) {
        ...
        fs.stat(`${this.video.dir}${file}`, (err, stat) => {
            if (err) {
                console.log(err);
            } else {
